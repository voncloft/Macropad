...
if ! grep -Eq "Found 0 unconnected (items|pads)" "$DRC_OUT"; then
  echo "FAIL: unconnected items found"
  cat "$DRC_OUT"
  exit 1
fi

echo "[2/7] Locked connector coordinates (J302/J303/J304)"
python3 - "$PCB" <<'PY'
import re, sys
...
echo "[3/7] Locked silk labels (J302/J303/J304 pin text)"
python3 - "$PCB" <<'PY'
import re, sys
from pathlib import Path
...
txt = Path(sys.argv[1]).read_text()
found = {}
for m in re.finditer(r'\(gr_text "([^"]+)"\s*\n\s*\(at\s+([-0-9.]+)\s+([-0-9.]+)\s+([-0-9.]+)\)', txt):
    label = m.group(1)
    found[label] = (float(m.group(2)), float(m.group(3)), float(m.group(4)))
...
for label, exp in expected.items():
    got = found.get(label)
    if got is None:
        print(f"FAIL: silk label missing: {label}")
        raise SystemExit(1)
    if any(abs(got[i] - exp[i]) > 0.02 for i in range(3)):
        print(f"FAIL: silk label moved: {label} got={got} expected={exp}")
        raise SystemExit(1)

print("PASS: J302/J303/J304 silk labels are locked")
PY

echo "[4/7] Z analytic check"
python3 "$ROOT/case/check_lcd_header_z_clearance.py" --header-height 8.8 --min-clearance 1.0

echo "[5/7] Z STL check"
python3 "$ROOT/case/check_lcd_header_z_stl.py" --header-height 8.8

echo "[6/7] Production artifacts exist"
for f in \
...
echo "[7/7] Schematic power-block warning gate"
